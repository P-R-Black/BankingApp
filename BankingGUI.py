...
def register():
    ...
    # Entries
    Entry(register_screen, textvariable=temp_name).grid(row=1, column=0, padx=2)
    Entry(register_screen, textvariable=temp_age).grid(row=2, column=0, padx=2)
    Entry(register_screen, textvariable=temp_gender).grid(row=3, column=0, padx=2)
    Entry(register_screen, textvariable=temp_password, show="*").grid(row=4, column=0, padx=2)
    ...
    for name in all_accounts:
        if name == login_name:
            ...
            if login_password == password:
                ...
                #Labels
                Label(account_dashboard, text="Account Dashboard", font=('Calibri', 12)).grid(row=0, sticky=N, pady=5)
                Label(account_dashboard, text=" Welcome " + name, font=('Calibri', 12)).grid(row=1, sticky=N)
                ...
                Label(account_dashboard).grid(row=1, sticky=N, pady=10)
                return
            else:
...
# Image Import
img = Image.open('DigitalBanking.png')
